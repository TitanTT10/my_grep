...
impl Config {
    fn new(args: &[String]) -> Result<Self, String> {
        ...
        if non_flag_args.is_empty() {
            return Err("Error: Pattern not provided.".to_owned());
        }

        let pattern = non_flag_args[0].clone();

        if non_flag_args.len() < 2 {
            return Err("Error: No input files provided.".to_owned());
        }

        let files = non_flag_args[1..].to_vec();

        return Ok(Self { pattern, files, case_insensitive });
    }
}
...
fn grep_file(file: &str, config: &Config) {
    match File::open(file) {
        ...
        Err(e) => { eprintln!("Could not open file: '{}': {}", file, e); }
    }
}
